...
class JanelaCadastro:
    def __init__(self, janela):
        ...
        if recuperacao_combobox == "Professor":
            botao = Button(self.frame, text="Salvar", command=self.cadastrar_professor)
            botao.grid(row=4, column=0, padx=5, pady=5, columnspan=2)

        elif recuperacao_combobox == "Aluno":
            botao = Button(self.frame, text="Salvar", command=self.cadastrar_aluno)
            botao.grid(row=4, column=0, padx=5, pady=5, columnspan=2)

        else:
            #mensagem de erro: messagebox
            pass
